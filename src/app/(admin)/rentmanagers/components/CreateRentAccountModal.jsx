...
const CreateRentAccountModal = ({ 
  ...
  // --- Tenant infinite-scroll state ---
  const [tenantsList, setTenantsList] = useState([]);
  const [tenantPage, setTenantPage] = useState(1);
  const [tenantHasMore, setTenantHasMore] = useState(true);
  const [loadingMoreTenants, setLoadingMoreTenants] = useState(false);
  const dropdownOptionsRef = useRef(null);

  // Fetch a page of tenants and append to the list
  const fetchTenantsPage = useCallback(async (page) => {
    if (!estateSlug || !user?.token) return;

    setLoadingMoreTenants(true);
    try {
      ...
      const newTenants = usersPayload?.data || [];
      const currentPage = usersPayload?.current_page ?? page;
      const lastPage = usersPayload?.last_page ?? page;
      ...
      setTenantPage(currentPage);
      setTenantHasMore(currentPage < lastPage && newTenants.length > 0);
    } catch (err) {
      console.error('Error fetching tenants page:', err);
      // Stop trying further pages on error, but keep whatever we already have
      setTenantHasMore(false);
    } finally {
      setLoadingMoreTenants(false);
    }
  }, [estateSlug, user]);

  // Handle scroll within the dropdown options list
  const handleTenantDropdownScroll = () => {
    const el = dropdownOptionsRef.current;
    if (!el || loadingMoreTenants || !tenantHasMore) return;

    const nearBottom = el.scrollTop + el.clientHeight >= el.scrollHeight - 40;
    if (nearBottom) {
      fetchTenantsPage(tenantPage + 1);
    }
  };
  ...
  useEffect(() => {
    if (show) {
      ...
      if (mode === 'cycle') {
        ...
        // Seed tenant list with whatever page 1 was passed in as a prop,
        // then reset paging state so scrolling fetches page 2 onward.
        const seedTenants = Array.isArray(tenants) ? tenants : [];
        setTenantsList(seedTenants);
        setTenantPage(1);
        setTenantHasMore(true);
      }
    }
  }, [show, mode, rentCycleToEdit, rentAccountForCycle]);
  ...
  const getFilteredTenants = () => {
    if (!tenantsList || !Array.isArray(tenantsList)) return [];
    if (!tenantSearchTerm.trim()) return tenantsList;
                      ...
                      {isTenantDropdownOpen && (
                        <div className="dropdown-menu show">
                          ...
                          <div
                            ...
                            onScroll={handleTenantDropdownScroll}
                          >
                            {filteredTenants.length > 0 ? (
                              <>
                                {filteredTenants.map(tenant => (
                                ...
                                {!tenantHasMore && !tenantSearchTerm && (
                          ...
                          <div className="dropdown-footer small text-muted">
                            {filteredTenants.length} tenant(s) loaded
                          </div>
                        </div>
                      )}
                    </div>
