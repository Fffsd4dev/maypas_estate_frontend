...
const CreateComplaintModal = ({ 
  ...
  useEffect(() => {
    if (show) {
      ...
      if (editMode && complaintToEdit) {
        console.log('Editing maintenance complaint:', complaintToEdit);
        // Get the maintenance ID - try different possible fields
        const maintenanceId = complaintToEdit.id || complaintToEdit.maintenance_id;
        console.log('Maintenance ID for editing:', maintenanceId);
        
        setFormData({
          title: complaintToEdit.maintenance_title || complaintToEdit.title || '',
          description: complaintToEdit.maintenance_description || complaintToEdit.description || '',
          priority: complaintToEdit.priority || 'medium',
          apartment_unit_uuid: complaintToEdit.apartment_unit_uuid || ''
        });
      } else {
  ...
  const handleAttachmentChange = (e) => {
    const file = e.target.files[0];
    if (file) {
      // Validate file type - you can expand this for different file types
      const validTypes = ['image/jpeg', 'image/jpg', 'image/png', 'image/gif', 'image/webp', 'application/pdf'];
      if (!validTypes.includes(file.type)) {
        setError('Please select a valid file (JPEG, PNG, GIF, WebP, PDF)');
        return;
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Get the maintenance ID for editing
      const maintenanceId = editMode ? (complaintToEdit.id || complaintToEdit.maintenance_id) : null;
      
      if (editMode && !maintenanceId) {
        throw new Error('Maintenance ID not found for editing');
      }
      ...
      console.log('Submitting maintenance complaint with data:', {
        title: formData.title,
        description: formData.description,
        priority: formData.priority,
        apartment_unit_uuid: formData.apartment_unit_uuid,
      ...
      if (editMode) {
        // Use PUT/POST method for editing and include maintenance ID in URL
        method = 'post'; // or 'put' depending on your API
        apiUrl = `${import.meta.env.VITE_BACKEND_URL}/api/${tenantSlug}/maintenance/update/${maintenanceId}`;
        console.log('Edit API URL:', apiUrl);
        
        // For edit, also send the apartment_unit_uuid in FormData if needed
        formDataToSend.append('apartment_unit_uuid', formData.apartment_unit_uuid);
      } else {
        // Use POST method for creating new maintenance complaint
        method = 'post';
        apiUrl = `${import.meta.env.VITE_BACKEND_URL}/api/${tenantSlug}/maintenance/create/${formData.apartment_unit_uuid}`;
        console.log('Create API URL:', apiUrl);
      }
      ...
      console.error('Maintenance complaint operation error:', err);
      console.error('Error response:', err.response?.data);
      console.error('Error status:', err.response?.status);
      setError(err.response?.data?.message || err.message || 
        (editMode ? 'Failed to update maintenance complaint' : 'Failed to create maintenance complaint'));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Modal show={show} onHide={handleModalClose} centered size="lg">
      <Modal.Header closeButton>
        <Modal.Title>
          {editMode ? 'Edit Maintenance Complaint' : 'Create New Maintenance Complaint'}
        </Modal.Title>
      </Modal.Header>
      <Form onSubmit={handleSubmit}>
        <Modal.Body>
          ...
          {success && (
            <Alert variant="success">
              {editMode ? 'Maintenance complaint updated successfully!' : 'Maintenance complaint created successfully!'}
            </Alert>
          )}
          ...
          <Row>
            <Col md={6}>
              <Form.Group className="mb-3">
                ...
                {editMode && (
                  <Form.Text className="text-muted">
                    Apartment unit cannot be changed when editing a maintenance complaint
                  </Form.Text>
                )}
              </Form.Group>
            </Col>
          ...
          <Form.Group className="mb-3">
            ...
            />
            <Form.Text className="text-muted">
              Upload an image or PDF as attachment (JPEG, PNG, GIF, WebP, PDF, max 5MB)
            </Form.Text>
            {editMode && complaintToEdit?.attachment && (
              <Form.Text className="text-info">
                Current attachment: {complaintToEdit.attachment}
              </Form.Text>
            )}
          </Form.Group>
          ...
          {attachment && (
            <Alert variant="info" className="d-flex justify-content-between align-items-center">
              ...
              <Button variant="outline-danger" size="sm" onClick={removeAttachment}>
                <IconifyIcon icon="mdi:close" />
              </Button>
            </Alert>
          )}
        ...
        <Modal.Footer>
          ...
          >
            {loading ? (
              ...
              <>
                <IconifyIcon icon={editMode ? "bx:save" : "bx:plus"} className="me-1" />
                {editMode ? 'Save Changes' : 'Create Maintenance Complaint'}
              </>
            )}
          </Button>
        </Modal.Footer>
